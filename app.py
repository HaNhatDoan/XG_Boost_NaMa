...
#        print(name)
        ...
        try:                                         #12
            ...
            # Bước 1: Lấy thêm các trường mới
        try:
            TIENSUGD = request.form['TIENSUGD']  # Tiền sử gia đình
        except:
            TIENSUGD = None  # hoặc gán mặc định

        try:
            TIENSUMT = request.form['TIENSUMT']  # Tiền sử mang thai
        except:
            TIENSUMT = None
        
        try:
            TONGIAO = request.form['TONGIAO'].strip().lower()
        except:
            TONGIAO = 'khong ro'  # hoặc None nếu không có
    
        Ton_Giao_luong = 1 if TONGIAO == 'luong' else 0
        Ton_Giao_phat = 1 if TONGIAO == 'phat' else 0
        Ton_Giao_thien_chua = 1 if TONGIAO == 'thien chua' else 0
        ...
        if os.path.exists(config.unsubmited_data_path):
            database = pd.read_excel(config.unsubmited_data_path, index_col=None, sheet_name='Sheet_name_1')
            df = pd.DataFrame(data)
            database = pd.concat([database, df], ignore_index=True)

        else:
            database = pd.DataFrame.from_dict(data)
        database.to_excel(config.unsubmited_data_path,sheet_name='Sheet_name_1', index = False)
...
def knowledgeTest():
    if request.method == 'POST':
        ...
        if os.path.exists(config.knowledgeTest_path):
            database = pd.read_excel(config.knowledgeTest_path,sheet_name='Sheet_name_1', index_col= None)
        else:
        ...
        database.to_excel(config.knowledgeTest_path,sheet_name='Sheet_name_1', index = False)
...
def loadModel():
    database = pd.read_excel(config.unsubmited_data_path, sheet_name='Sheet_name_1', index_col=None)
    patient_ID = request.args.get('patient_ID',type = str)
...
def info_display():
    ...
    if request.method == 'POST':
        ...
        if os.path.exists(config.submited_data_path):
            submit_database = pd.read_excel(config.submited_data_path, sheet_name='Sheet_name_1', index_col=None)
            submit_database = pd.concat([submit_database, patient_data], ignore_index=True)
        else:
            submit_database = patient_data
        submit_database.to_excel(config.submited_data_path,sheet_name='Sheet_name_1', index = False)
        return redirect('/getInfo')
    else:
        database = pd.read_excel(config.unsubmited_data_path, sheet_name='Sheet_name_1', index_col=None)
        ...
        def pregnant_history(x):
            if x==0:
                ...
                return 'Bị nám má khi mang thai từ tháng thứ %s'
        ...
        knowledge_database = pd.read_excel(config.knowledgeTest_path, sheet_name='Sheet_name_1', index_col=None)
